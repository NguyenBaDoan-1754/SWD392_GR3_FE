...
import { Search, RefreshCw, TrendingUp, TrendingDown, BarChart2 } from "lucide-react";
...
export default function UserMarketContent() {
  ...
  const displayCount = useMemo(() => stockPrices.slice(0, 15).length, [stockPrices]);
    ...
    <div className="min-h-full bg-slate-950 py-8 px-6 md:px-10">
      ...
      >
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white tracking-tight">Thị Trường Chứng Khoán</h1>
            <p className="text-slate-400 text-sm">Theo dõi giá cổ phiếu theo thời gian thực</p>
          </div>
        </div>
      </motion.div>
      ...
      <div className="text-slate-500 text-sm mb-4">
        {!loading && !error && (
          <span>Hiển thị <span className="text-slate-300 font-medium">{displayCount}</span> / <span className="text-slate-300 font-medium">{totalElements}</span> kết quả</span>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-rose-400 font-medium mb-1">Không thể tải dữ liệu</p>
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-slate-400 font-medium">Không tìm thấy cổ phiếu nào</p>
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            {stockPrices.slice(0, 15).map((price, i) => {
              const change = price.open > 0 ? ((price.close - price.open) / price.open) * 100 : 0;
              const isUp = change >= 0;
              return (
                <motion.div
                ...
                >
                  {/* Glow effect */}
                  <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none rounded-2xl ${isUp ? "bg-emerald-500/3" : "bg-rose-500/3"}`} />
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <div>
                          <p className="text-white font-bold text-sm">{price.tickerSymbol.toUpperCase()}</p>
                          <p className="text-slate-500 text-xs">{new Date(price.tradingDate).toLocaleDateString("vi-VN")}</p>
                        </div>
                      </div>
                    </div>
                    <div className={`flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-lg ${isUp ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400"}`}>
                  ...
                  <p className="text-2xl font-bold text-white mb-3">
                    {(price.close / 1000).toFixed(2)}<span className="text-sm font-normal text-slate-500 ml-1">K</span>
                  </p>

                  <div className="grid grid-cols-3 gap-2 pt-3 border-t border-slate-800/80">
                    {[
                      { label: "Mở cửa", value: (price.open / 1000).toFixed(1) + "K" },
                      { label: "Cao nhất", value: (price.high / 1000).toFixed(1) + "K" },
                      { label: "KL", value: (price.volume / 1000000).toFixed(1) + "M" },
                    ].map((stat) => (
                      <div key={stat.label} className="text-center">
                        <p className="text-slate-500 text-xs mb-0.5">{stat.label}</p>
                        <p className="text-slate-300 text-xs font-medium">{stat.value}</p>
                      </div>
                    ))}
                  </div>
                </motion.div>
